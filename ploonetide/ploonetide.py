...
class TidalSimulation(Simulation):
    """Subclass that defines a tidal simulation
    """
    ...
    def __init__(self, activation_energy=3E5, melt_fraction=0.5, heat_capacity=1260,
                 ...
                 planet_rotperiod=15.656, planet_eccentricity=0.1, planet_rigidity=4.46E10,
        ...
        Args:
            activation_energy (float, optional): Description
            melt_fraction (float, optional): Description
            heat_capacity (int, optional): Description
            melt_fraction_coeff (int, optional): Description
            solidus_temperature (int, optional): Description
            breakdown_temperature (int, optional): Description
            liquidus_temperature (int, optional): Description
            thermal_conductivity (int, optional): Description
            Rayleigh_critical (int, optional): Description
            flow_geometry (int, optional): Description
            thermal_expansivity (float, optional): Description
            planet_size_evolution (bool, optional): Description
            planet_internal_evolution (bool, optional): Description
            planet_core_dissipation (bool, optional): Description
            star_internal_evolution (bool, optional): Description
            star_mass (int, optional): Description
            star_radius (int, optional): Description
            star_eff_temperature (int, optional): Description
            star_saturation_rate (float, optional): Description
            star_angular_coeff (float, optional): Description
            star_rotperiod (int, optional): Description
            star_alpha (float, optional): Description
            star_beta (float, optional): Description
            star_age (int, optional): Description
            sun_omega (float, optional): Description
            sun_mass_loss_rate (float, optional): Description
            planet_mass (int, optional): Description
            planet_radius (None, optional): Description
            planet_angular_coeff (float, optional): Description
            planet_orbperiod (None, optional): Description
            planet_rotperiod (float, optional): Description
            planet_eccentricity (float, optional): Description
            planet_rigidity (float, optional): Description
            planet_alpha (TYPE, optional): Description
            planet_beta (TYPE, optional): Description
            moon_radius (int, optional): Description
            moon_mass (int, optional): Description
            moon_albedo (float, optional): Description
            moon_eccentricty (float, optional): Description
            moon_semimaxis (None, optional): Description
            system (str, optional): Description
        """
        # ************************************************************
        ...
        # ************************************************************
        self.__planet_size_evolution = planet_size_evolution
        self.__planet_internal_evolution = planet_internal_evolution
        self.__planet_core_dissipation = planet_core_dissipation
        self.__star_internal_evolution = star_internal_evolution
        ...
        # General constants
        self.activation_energy = activation_energy  # J mol^-1  -->  kg * m^2 s^-2 * mol^-1
        self.melt_fraction = melt_fraction  # No unit
        self.heat_capacity = heat_capacity  # J kg^-1 K^-1  -->  m^2 s^-2 K^-1
        self.melt_fraction_coeff = melt_fraction_coeff  # No unit
        self.solidus_temperature = solidus_temperature  # K
        self.breakdown_temperature = breakdown_temperature  # K
        self.liquidus_temperature = liquidus_temperature  # K
        self.thermal_conductivity = thermal_conductivity  # W m^-1 K^-1  -->  kg m s^-3 K^-1
        self.Rayleigh_critical = Rayleigh_critical  # No unit
        self.flow_geometry = flow_geometry  # No unit
        self.thermal_expansivity = thermal_expansivity  # K^-1
        ...
        self.star_eff_temperature = star_eff_temperature  # Effective temperature Cañas et. al (2020)
        self.star_rotperiod = star_rotperiod * DAY
        self.star_saturation_rate = star_saturation_rate
        self.star_saturation_period = 2. * np.pi / self.star_saturation_rate  # Saturation spin period
        self.star_angular_coeff = star_angular_coeff
        self.star_alpha = star_alpha  # This is Star's alpha
        self.star_beta = star_beta   # star's core fraction
        ...
        self.star_luminosity = luminosity(self.star_radius, self.star_eff_temperature)  # Stellar luminosity
        ...
        self.planet_rotperiod = planet_rotperiod * HOUR  # Initial planetary rotational period
        ...
        self.moon_mass = moon_mass * MEARTH  # Earth
        self.moon_radius = moon_radius * REARTH  # Earth
        ...
        # Arguments
        self.args = dict(
            star_internal_evolution=self.__star_internal_evolution, star_k2q=self.star_k2q,
            planet_internal_evolution=self.__planet_internal_evolution, planet_k2q=self.planet_k2q,
            planet_size_evolution=self.__planet_size_evolution, Rp=self.planet_radius,
            planet_core_dissipation=self.__planet_core_dissipation
        )
        ...
        self.parameters = dict(Ms=self.star_mass, Rs=self.star_radius, Ls=self.star_luminosity,
                               ...
                               E_act=self.activation_energy, B=self.melt_fraction_coeff,
                               Ts=self.solidus_temperature, Tb=self.breakdown_temperature,
                               Tl=self.liquidus_temperature, Cp=self.heat_capacity,
                               ktherm=self.thermal_conductivity, Rac=self.Rayleigh_critical,
                               a2=self.flow_geometry, alpha_exp=self.thermal_expansivity,
        ...
        print(self.planet_orbperiod / DAY, "days", self.moon_rotperiod / DAY, "days",
              self.star_mass / MSUN, "Msun", self.planet_mass / PLANETS.Jupiter.M, "Mjup",
              self.planet_radius / PLANETS.Jupiter.R, "Rjup")
